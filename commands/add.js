...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // permission: must be manager or admin
    ...
    if (!isAdmin && !isManager) return interaction.reply({ content: "❌ You cannot add to waitlists.", ephemeral: true });
    ...
    if (data.users.includes(user.id)) return interaction.reply({ content: "⚠️ User is already on the list.", ephemeral: true });
    ...
    if (config.logChannelId) {
      ...
      if (ch) ch.send(`➕ <@${user.id}> added to **${name}** by <@${interaction.user.id}> — <t:${Math.floor(Date.now()/1000)}:F>`).catch(()=>{});
    }

    return interaction.reply({ content: `✅ Added **${user.tag}** to **${name}**.`, ephemeral: true });
  }
};
